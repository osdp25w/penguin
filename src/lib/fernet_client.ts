...
function wordArrayFrom(bytes: Uint8Array): any {
  const CryptoJS: any = (globalThis as any).CryptoJS
...
function bytesFromWordArray(wa: any): Uint8Array {
...
export function fernetEncrypt(plain: string, base64UrlKey: string, timestamp?: number): string {
  const CryptoJS: any = (globalThis as any).CryptoJS
...
export function fernetDecrypt(token: string, base64UrlKey: string): string {
  const CryptoJS: any = (globalThis as any).CryptoJS
  if (!CryptoJS) throw new Error('CryptoJS not loaded')
  const keyBytes = b64uToBytes(base64UrlKey)
  if (keyBytes.length !== 32) throw new Error('Invalid Fernet key length')
  ...
  if (bytes.length < 1 + 8 + 16 + 32) throw new Error('Invalid token length')
  if (bytes[0] !== 0x80) throw new Error('Unsupported Fernet version')
  ...
  for (let i = 0; i < macBytes.length && i < macCalcBytes.length; i++) ok = ok && (macCalcBytes[i] === macBytes[i])
