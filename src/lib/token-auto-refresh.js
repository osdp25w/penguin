 ...
 */
export function startAutoRefresh(intervalMinutes = 10, refreshThresholdMinutes = 15) {
    if (isInitialized) {
 ...
 * 檢查並刷新 Token（如果需要）
 */
async function checkAndRefreshToken(thresholdMinutes) {
    // 延遲引入避免循環依賴
 ...
 * 手動觸發 Token 檢查和刷新
 */
export function manualRefreshCheck(thresholdMinutes = 15) {
    return checkAndRefreshToken(thresholdMinutes);
}
/**
 * 在頁面可見性變化時檢查 Token
 */
export function setupVisibilityChangeRefresh(thresholdMinutes = 5) {
