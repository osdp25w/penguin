...
function generateMockAlert(siteId, index) {
...
function generateAlerts(siteId, since) {
...
function generateGeneralAlerts() {
    // 總是返回靜態測試數據，確保有內容顯示
...
export const alertsHandlers = [
    rest.get('/api/v1/alerts', (req, res, ctx) => {
        ...
        try {
            ...
            return res(ctx.delay(300), ctx.status(200), ctx.json(validatedAlerts));
        }
        catch (error) {
            console.error('[MSW] Alerts API error:', error);
            return res(ctx.status(400), ctx.json({ error: error.message || 'Failed to generate alerts' }));
        }
    }),
    ...
    rest.patch('/api/v1/alerts/:id', (req, res, ctx) => {
        ...
        return res(ctx.delay(200), ctx.status(200), ctx.json({ success: true, id }));
    })
];
// 提供給前端在無法啟動 Service Worker（如非安全來源或子路徑部署）時的後備方法
export function getDemoAlerts(filter) {
    let alerts = generateGeneralAlerts();
    if (filter === 'open')
