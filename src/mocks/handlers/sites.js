...
// 生成隨機站點（不含真實地名）
function generateMockSite(region, index) {
...
// 生成站點資料
function generateSites(region) {
    if (!SEED_MOCK_ENABLED) {
        return [];
    }
    const regions = region ? [region] : ['hualien', 'taitung'];
    const sites = [];
    regions.forEach(r => {
...
export const sitesHandlers = [
    rest.get('/api/v1/sites', (req, res, ctx) => {
        try {
            ...
            return res(ctx.delay(delay), ctx.status(200), ctx.json(sites));
        }
        catch (error) {
