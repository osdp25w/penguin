...
export const rentalsHandlers = [
  ...
  rest.post(api('/rentals'), async (req, res, ctx) => {
    try {
      ...
      return res(
        ctx.delay(faker.number.int({ min: 800, max: 1500 })),
        ctx.json(rental)
      )
  ...
  rest.post(api('/rentals/:rentalId/unlock'), async (req, res, ctx) => {
    ...
    if (unlockFails) {
      return res(
        ctx.status(500),
        ctx.json({ message: '開鎖失敗，請重試或聯絡客服' })
      )
    }
    ...
    return res(
      ctx.delay(faker.number.int({ min: 500, max: 1200 })),
      ctx.json({
        message: '開鎖成功',
        state: rental.state,
        rental
  ...
  rest.post(api('/rentals/:rentalId/cancel'), async (req, res, ctx) => {
    ...
    return res(
      ctx.delay(faker.number.int({ min: 300, max: 800 })),
      ctx.json({
        message: '租借已取消',
        rental
      })
  ...
  rest.get(api('/rentals/:rentalId'), (req, res, ctx) => {
    ...
    return res(
      ctx.delay(faker.number.int({ min: 200, max: 500 })),
      ctx.json(rental)
    )
  }),
