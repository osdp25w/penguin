...
* 瀏覽**系統總覽**：查看線上車輛數、總行駛距離、減碳量等關鍵指標，並透過互動式圖表監控營運狀況。
* 使用**場域地圖**：在地圖上監控各站點中車輛的分佈狀況（整合 NLSC EMAP 地圖服務），並支援完整的**車輛歸還流程**。
* 查看**車輛清單**：以表格方式檢視所有車輛資訊，支援狀態篩選、批次維護操作等進階功能。
* 監控**警報中心**：即時查看車輛警報（例如過熱、異常訊號），支援嚴重等級分類與詳細處理建議。
* 使用**機器學習綜合預測**：輸入參數獲得預測結果，例如路線規劃建議、預估行程時間與能耗、減碳量以及電池故障風險等。
* **帳號管理**：管理員專用頁面，支援使用者帳號與角色管理（新增帳號、設定角色權限、啟用/停用帳號）。

## 🚀 新增功能亮點

### 車輛歸還流程
- **完整歸還工作流**：從站點選擇到車輛確認的完整流程
- **表單驗證**：使用 Zod 進行嚴格的資料驗證
- **即時狀態更新**：歸還過程的即時狀態反饋
- **錯誤處理**：完善的錯誤處理與重試機制

### 現代化設計系統
- **統一視覺風格**：基於 Tailwind CSS 的設計代幣系統
- **響應式介面**：完全響應式設計，支援各種螢幕尺寸
- **無障礙支援**：遵循 WCAG 無障礙設計指引
- **圖示系統**：整合 Phosphor Icons 提供一致的視覺語言
...
* **車輛歸還系統**：
  ...
  * `GET /api/v1/sites/{siteId}/vehicles` – 獲取指定站點的可歸還車輛列表。回傳該站點所有車輛的基本資訊。
  * `POST /api/v1/returns` – 提交車輛歸還請求。需包含 `vehicleId`、`siteId`、`odometer`（里程數）、`battery`（電量百分比）等必填欄位，支援 `photos`（照片）和 `notes`（備註）等選填欄位。
  * `GET /api/v1/returns` – 獲取歸還記錄列表。支援 `vehicleId` 和 `siteId` 篩選參數，以及分頁參數。
  * `GET /api/v1/returns/stats` – 獲取歸還統計資料。支援日期範圍查詢，回傳成功率、平均里程、營收統計等資訊。
  * `DELETE /api/v1/returns/{returnId}` – 刪除指定的歸還記錄（管理員權限）。
...
* **單元測試框架**：使用 Vitest 進行 Vue 元件和 Pinia 狀態管理的單元測試
  - 測試 Pinia stores 的狀態管理邏輯
  - 測試 Vue 元件的渲染和互動行為
  - 模擬 API 請求和錯誤處理
  
* **端對端測試框架**：使用 Playwright 進行完整使用者流程測試
  - 測試頁面間的導航功能
  - 測試車輛歸還完整流程
  - 測試使用者管理功能
  - 測試警報中心互動
...
#### 單元測試
- **Returns Store**：車輛歸還狀態管理、API 整合、表單驗證
- **Return Modal Component**：表單互動、驗證邏輯、事件處理

#### E2E 測試
- **導航測試**：主選單導航、路由保護、頁面載入
- **車輛歸還流程**：模態開啟、表單填寫、提交流程
- **使用者管理**：使用者列表、新增使用者、權限控制
- **警報中心**：警報列表、篩選功能、詳情查看
- **ML 預測**：預測介面、重新計算功能

### 測試最佳實務

- 使用 `data-test` 屬性標記測試選擇器
- 模擬 API 請求以避免依賴外部服務
- 測試覆蓋正常流程和異常情況
- 定期執行測試以確保功能穩定性
